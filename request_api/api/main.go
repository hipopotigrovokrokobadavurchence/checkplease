...
func GetPendingRequests(placeID int) (requests []request, err error) {
	...
	rows, err := tx.Query(`SELECT r.id, r.table_id,  r.menu_item_id, r.place_id, t.id_hash, mi.name, r.Inserted_At
                        FROM requests r
												join tables t on r.table_id = t.id
												join menu_items mi on r.menu_item_id = mi.id
                        WHERE r.place_id = $1 and r.state_id = 10 order by r.id`, placeID)
	if err != nil {
		glog.Error(err)
		tx.Rollback()
		return requests, err
	...
	for rows.Next() {
		...
		err = rows.Scan(&request.ID, &request.TableID, &request.MenuItemID, &request.PlaceID, &request.TableNumber,
			&request.MenuItemName, &request.InsertedAt)
		if err != nil {
			tx.Rollback()
			return requests, err
		}
